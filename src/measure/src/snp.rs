...
/// Calculate the OVMF hash from OVMF file bytes
pub fn calc_snp_ovmf_hash(ovmf: Vec<u8>) -> Result<SnpLaunchDigest, MeasurementError> {
    let ovmf = OVMF::new(ovmf)?;
...
pub fn snp_calc_launch_digest(
    ...
    let ovmf = OVMF::new(snp_measurement.ovmf_bytes)?;
    ...
    let sev_hashes = match snp_measurement.kernel_bytes {
        Some(kernel) => Some(SevHashes::new(
            kernel,
            snp_measurement.initrd_bytes,
